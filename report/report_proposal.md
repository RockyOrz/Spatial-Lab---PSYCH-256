...
为了更高效、可复现地生成与扩展题目集，我们使用 Google AI Studio 并借助刚发布不久的 Gemini 3 Pro（前端代码能力较强）快速搭建了一个前端 demo，用于批量生成基于“渲染法”的测试题；该工具帮助我们标准化不同条件下的题目生成流程、统一题目格式/命名/元数据标准，并显著提升迭代效率。随后，我们利用 agent 技术（gpt-codex系列）编写了 `scripts/run_llm_trials.py`，对同一批 40 道测试题批量调用 4 个模型并自动量化：统一提示词、逐题独立会话采样，收集答案与置信度，并将结果汇总追加到 `report/trials_result.md` 以支持后续统计分析。
